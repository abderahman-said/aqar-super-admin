...
import { getAgentsAdminDeleted, RestoreUser } from "../store/PropertySlice";

const DelAgent = () => {
  const dispatch = useDispatch();
  const toast = useRef(null);

  const { getAgentsAdminDeletedData } = useSelector(
  ...
  useEffect(() => {
    ...
    const state = true;
    dispatch(getAgentsAdminDeleted({ page, state }));
  }, [dispatch, first, rows]);
  ...
  return (
    <>
      ...
      <DataTable
        value={getAgentsAdminDeletedData || []}
        dataKey="_id"
        onPage={onPageChange}
