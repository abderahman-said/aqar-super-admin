...
const EmairitesAdd = ({ visible, setVisible, selectedEmirate }) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      nameEn: selectedEmirate ? selectedEmirate.name.en : "",
      nameAr: selectedEmirate ? selectedEmirate.name.ar : "",
    },
    validate: (data) => {
      const errors = {};
      if (!data.nameEn) {
    ...
    onSubmit: (data) => {
      ...
      if (selectedEmirate) {
        dispatch(getUpdateLocation({ id: selectedEmirate.id, data: date }))
          .unwrap()
          .then(() => {
