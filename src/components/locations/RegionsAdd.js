...
const RegionsAdd = ({ visible, setVisible, id, selectedRegions }) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      nameEn: selectedRegions ? selectedRegions.name.en : "",
      nameAr: selectedRegions ? selectedRegions.name.ar : "",
    },
    validate: (data) => {
      const errors = {};
      if (!data.nameEn) {
    ...
    onSubmit: (data) => {
      ...
      if (selectedRegions) {
        dispatch(getUpdateLocation({ id: selectedRegions.id, data: date }))
          .unwrap()
          .then(() => {
