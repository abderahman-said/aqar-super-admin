...
const Step3Edit = ({
  ...
  const { mutate: UpdateProperties } = useUpdateProperties(id);
  ...
  const formik2 = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (!selectedLocation) delete updatedFormData.location;

      if (formData.images === values.images) {
        delete updatedFormData.images;
      }
      setFormData(updatedFormData);

      UpdateProperties(updatedFormData);
  ...
  console.log("formik2.values.images", formik2.values.images);
      ...
      <div className="grid">
        {locationView && locationView.length > 0 && (
          <div className={styles.title_location}>
            Locations:{" "}
            <span> {locationView.map((ele) => ele?.name?.en).join(", ")}</span>
          </div>
        )}
        ...
        <div className="col-12 md:col-12 lg:col-12 ">
          ...
          {formik2.values.images.length > 0 && (
            <div className={styles.image_preview}>
              <div className={styles.images_container}>
                {formik2.values.images.map((img, index) => (
                  <div key={index} className={styles.image_preview_item}>
                    <Image
                      src={img}
                      alt={`uploaded-${index}`}
